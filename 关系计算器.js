...
//基于https://github.com/mumuy/relationship
export class relation_calculate extends plugin {
    constructor() {
        super({
            name: '关系计算器',
            dsc: '计算中国家庭称谓',
            ...
            rule: [
                {
                    reg: '^(\\S+?)(应该|得)?(称呼|叫|喊)(\\S+?)(什么|啥)？?$',
                    fnc: 'calculate'
                },
                {
                    reg: '^(\\S+?)(应该|得)?(如何|怎么)(称呼|叫|喊)(\\S+?)？?$',
                    fnc: 'calculate'
                },
                {
                    reg: '^(\\S+?)是(\\S+?)的(谁|什么|什么亲戚|啥亲戚|什么人)？?$',
                    fnc: 'calculate'
                },
                {
                    reg: '^(\\S+?)(应该|得)?管(\\S+?)叫(什么|啥)？?$',
                    fnc: 'calculate'
                },
                {
                    reg: '^(\\S+?)(和|与)(\\S+?)(之间)?是(什么|啥)关系？?$',
                    fnc: 'calculate'
                },
                {
                    reg: '^(\\S+?)对于(\\S+?)是(什么|啥)关系？?$',
                    fnc: 'calculate'
                },
                {
                    reg: '^(\\S+?)(指的)?是(什么|啥)(意思|关系|亲戚关系|辈分|人)？?$',
                    fnc: 'calculate'
                },
                {
                    reg: '^(\\S+?)(应该|得)?(称呼|叫|喊)(什么|啥)？?$',
                    fnc: 'calculate'
                },
                {
                    reg: '^(\\S+?)(应该|得)?(如何|怎么)(称呼|叫|喊)？?$',
                    fnc: 'calculate'
                },
                {
                    reg: '^(\\S+?)是(谁|什么|什么亲戚|啥亲戚|什么人|什么辈分|啥辈分)？?$',
                    fnc: 'calculate'
                }
            ]
        })
    ...
    async calculate(e) {
        let msg = e.msg
        let result = relationship(msg.replace(/^#/, ''));
        if(result.length == 0) {
            await e.reply('计算失败,请确认命令正确性,示例:\n我应该叫外婆的哥哥什么？\n七舅姥爷应该叫我什么？\n舅公是什么亲戚\n舅妈如何称呼外婆？\n外婆和奶奶之间是什么关系？', true)
        } else {
            await e.reply(result.join('或'), true)
        }
        return true
    }
}
...
const relationship = (function() {
    ...
    var s = [{
        ...
        opt: s=>({
            text: s[1],
            target: s[2],
            type: s[1].indexOf("的") > -1 ? "default" : "chain"
        })
    }, {
    ...
    function b(s) {
        ...
        if (x[s])
            ...
            let[x,d=0] = s.replace(/^十/, "一十").split("十").map((s=>m.indexOf(s))).reverse();
            f = 10 * d + x
        }
        return f
    }
    ...
    let o = Object.assign({}, {
        ...
        "[w|s|d]": ["妻儿", "妻小"],
        "[w|d]": ["妻女"],
        "[s|d]": ["子女", "儿女", "小孩", "孩子", "孩子们", "孩儿", "小孩儿", "宝宝", "宝贝", "娃", "娃子", "儿辈", "子辈", "细伢", "伢子", "子息", "子嗣", "崽", "幺儿"],
        "[s|d],[s|d]": ["孙辈", "孙息", "孙枝"],
        "s,[s|d]": ["孙子女"],
        "d,[s|d]": ["外孙子女"],
        "s#xb,s": ["子侄"],
      ...
      , e = function(s, f) {
        ...
        for (let d in f) {
            ...
            for (let f in s[m]) {
                let h = s[m][f]
                  , l = d.replace(m, f);
                if (!["h,h", "w,w", "w,h", "h,w"].some((s=>l.includes(s)))) {
                    let s = [];
                    h.forEach((function(f) {
                        b.forEach((function(x) {
                            x.includes("?") ? s.push(x.replace("?", f)) : s.push(f + x)
                        }
                        ))
                    }
                    )),
    ...
    for (let s in l) {
        t[s] = {};
        for (let f in l[s])
            -1 == f.indexOf("]") && (t[s][f] = l[s][f])
    }
    let r = {};
    for (let s in l) {
        r[s] = {};
        for (let f in l[s])
            f.indexOf("]") > -1 && N(f).forEach((function(x) {
                r[s][x] = l[s][f]
            }
            ))
    }
    o = Object.assign({}, o, e(t, M), e(r, M));
    for (let s in w)
        o[s] = [].concat(w[s], o[s] || []);
    o.o = ["passer-by.com", "作者"];
    const G = {
        w: ["妻", "内", "岳", "岳家", "丈人"],
        h: ["夫", "外", "公", "婆家", "婆婆"]
    };
    let c = new Set(Object.values(o).flat());
    for (let s in o)
        if (s.match(/^[fm]/) || s.match(/^[olx][bs]$|^[olx][bs],[^mf]/))
            for (let f in G) {
                let x = f + "," + s;
                if (s.match(/[fm]/)) {
                    let s = x.replace(/,[ol]([sb])(,[wh])?$/, ",x$1$2").replace(/(,[sd])&[ol](,[wh])?$/, "$1$2");
                    if (s != x && o[s])
                        continue
                }
                o[x] || (o[x] = []);
    ...
    function i(s, f) {
        let x = {
          ...
          , d = "";
        if (s.match(/&o$/) ? d = "&l" : s.match(/&l$/) && (d = "&o"),
        s) {
            s = s.replace(/&[ol\d+]/g, ""),
            f < 0 && (s.match(/^w/) ? f = 1 : s.match(/^h/) && (f = 0));
            let m = []
              , b = function(f) {
                let m = ("," + f + "," + s).replace(/,[fhs]|,[olx]b/g, ",1").replace(/,[mwd]|,[olx]s/g, ",0");
            ...
            return f < 0 ? (m.push(b(1)),
            m.push(b(0))) : m.push(b(f)),
            m
        }
        return [""]
    }
    function $(s) {
        let f = s.filter((s=>s == s.replace(/[ol](?=[s|b])/g, "x").replace(/&[ol]/, "")));
        return s.filter((s=>{
            let x = s.replace(/[ol](?=[s|b])/g, "x").replace(/&[ol]/, "");
            return f.includes(s) || s != x && !f.includes(x)
        }
        )).filter(((s,f,x)=>x.indexOf(s) === f))
    }
    function W(s) {
        let f = {
            f: 1,
    ...
    function u(s) {
        ...
        if (s.match(/&([\d]+)(,[hw])?$/)) {
            ...
            p[s]) {
                let d = "";
                W(s) < 3 && !s.match(/[hw],/) ? (p[s].forEach((function(s) {
                    !d && s.includes("几") && (d = s.replace("几", x))
                }
                )),
                d || (d = p[s][0].match(/^[大小]/) ? p[s][0].replace(/^[大小]/, x) : x + p[s][0])) : d = p[s][0],
                f.push(d)
            }
        } else
            s = s.replace(/&\d+/g, "");
        if (f.length || (f = x(s)),
        f.length || (f = x(s = s.replace(/&[ol]/g, ""))),
        f.length || (f = x(s = s.replace(/[ol](b|s)/g, "x$1"))),
        !f.length) {
            let d = s.replace(/x/g, "l")
              , m = s.replace(/x/g, "o");
            f = f.concat(x(m), x(d))
        }
        return f
    }
    let g = Object.assign({}, p, {
        xb: ["兄弟"],
        xs: ["姐妹"]
    });
    function E(s, f) {
        let x = s.split(",").map((function(s) {
            let f = s.replace(/&[ol\d]+/, "");
            return g[f][0]
        }
        )).join("的");
        return f && f > -1 && g[f + "," + s] && (0 == f ? x = "(女性)" + x : 1 == f && (x = "(男性)" + x)),
        x
    }
    let j = Object.assign({}, p, {
        "f,xb": ["伯叔父", "几爸爸", "几爸", "几爹"],
      ...
      , S = {};
    for (let s in j)
        j[s].forEach((function(f) {
            void 0 === S[f] && (S[f] = []),
            S[f].push(s)
        }
        ));
    let O = function(s) {
        let {from: f, to: x, sex: d} = s
          , m = s.from.split(",")
          , b = s.to.split(",");
        for (let h = 0; h < m.length && h < b.length; h++) {
            if (m[h] != b[h]) {
                if (W(m[h]) == W(b[h]) && m[h].match(/^[xol][bs]|^[sd]/) && b[h].match(/^[xol][bs]|^[sd]/)) {
                    if (m[h].replace(/&([ol\d]+)/, "").replace(/^[xol]([bs])/, "$1") != b[h].replace(/&([ol\d]+)/, "").replace(/^[xol]([bs])/, "$1"))
                        break;
                    let l = m[h].match(/&([ol\d]+)/)
                      , M = b[h].match(/&([ol\d]+)/);
                    l || (l = m[h].match(/([ol])[bs]/)),
                    M || (M = b[h].match(/([ol])[bs]/));
                    let w = l ? l[1] : ""
                      , o = M ? M[1] : "";
                    if (w && o)
                        isNaN(w) || isNaN(o) ? !isNaN(w) && "o" == o || "l" == w && !isNaN(o) ? m[h] = m[h].replace(/^[xol]b|^s/, "lb").replace(/^[xol]s|^d/, "ls") : (!isNaN(w) && "l" == o || "o" == w && !isNaN(o)) && (m[h] = m[h].replace(/^[xol]b|^s/, "ob").replace(/^[xol]s|^d/, "os")) : +w > +o ? m[h] = m[h].replace(/^[xol]b|^s/, "lb").replace(/^[xol]s|^d/, "ls") : +w < +o && (m[h] = m[h].replace(/^[xol]b|^s/, "ob").replace(/^[xol]s|^d/, "os")),
                        f = m.slice(h).join(","),
                        x = b.slice(h + 1).join(","),
                        d = b[h].match(/^([fhs1](&[ol\d]+)?|[olx]b)(&[ol\d]+)?/) ? 1 : 0;
                    else if (s.optimal && (l = m[h].match(/([xol])[bs]/),
                    M = b[h].match(/([xol])[bs]/),
                    w = l ? l[1] : "",
                    o = M ? M[1] : "",
                    "x" == w || "x" == o)) {
                        f = m.slice(h + 1).join(","),
                        x = b.slice(h + 1).join(","),
                        d = m[h].match(/^([fhs1](&[ol\d]+)?|[olx]b)(&[ol\d]+)?/) ? 1 : 0;
                        continue
                    }
                }
                break
    ...
    function y(s) {
        (s = s.replace(/之/g, "的").replace(/吾之?(.+)/, "$1").replace(/我的?(.+)/, "$1")).match(/[^娘婆岳亲]家的?(孩子|儿子|女儿)/) && (s = s.replace(/家的?/, "的"));
        let f = (s = (s = (s = s.replace(/(舅|姑)+(爸|父|丈|妈|母)?家的?(哥|姐|弟|妹)+/, "$1表$3").replace(/(舅|姑)+(爸|父|丈|妈|母)?家的?/, "$1表")).replace(/(伯|叔)+(父|母)?家的?(哥|姐|弟|妹)+/, "堂$3").replace(/(伯|叔)+(父|母)?家的?/, "堂")).replace(/姨+(爸|父|丈|妈|母)?家的?(哥|姐|弟|妹)+/, "姨$2").replace(/姨+(爸|父|丈|妈|母)?家的?/, "姨")).split("的")
          , m = []
          , h = !0;
        for (; f.length; ) {
            let s = f.shift()
            ...
            M.forEach((function(s) {
                let f = (s = s.replace(/^[尕幺细满碎晚末尾幼]/, "小")).match(/^[大|小]|^[一|二|三|四|五|六|七|八|九|十]+/);
                if (f) {
                    let x = s.replace(f[0], "几")
                      , d = s.replace(f[0], "")
                      , m = b(f[0]);
                    [x, d, s].forEach((function(s, f) {
                        let x = S[s];
                        x && x.length && x.forEach((function(x) {
                            let d = x.replace(/(,[hw])$/, "&" + m + "$1").replace(/([^hw]+)$/, "$1&" + m);
                            x.match(/^[mf,]+$/) || s.match(/^[从世]/) || o[f].push(d)
                        }
                        ))
                    }
                    ))
          ...
          , m = function(s) {
            ...
            if (!d[s]) {
                ...
                do {
                    b = s;
                    for (let x of f)
                        if ((s = s.replace(x.exp, x.str)).includes("#"))
                            return s.split("#").forEach(m),
                            !1
                } while (b != s);
                if (s.match(/,[mwd0](&[ol\d+])?,w|,[hfs1](&[ol\d]+)?,h/))
                    return !1;
                x.push(s)
            }
        };
    ...
    function v(s, f) {
        ...
        if (f > -1 && !s.includes(",1") && !s.includes(",0") && (s = "," + f + s),
        s.match(/,[mwd0](&[ol\d]+)?,w|,[hfs1](&[ol\d]+)?,h/))
            return [];
        let x = N(s).map((function(s) {
            ...
            m.forEach((function(f) {
                (function(s) {
                    ...
                    if (d < 0) {
                        let s = -1
                          , m = -1;
                        if (f.match(/^,[w1]/) ? m = 1 : f.match(/^,[h0]/) && (m = 0),
                        x.match(/^,[w1]/) ? s = 1 : x.match(/^,[h0]/) && (s = 0),
                        -1 == m && s > -1)
                            d = s;
                        else if (m > -1 && -1 == s)
                            d = m;
                    ...
                    return m.forEach((function(f) {
                        b.forEach((function(x) {
                            let m = d
                              , b = "," + x;
                            if (b.match(/,([fhs1](&[ol\d]+)?|[olx]b)(&[ol\d]+)?$/) && (m = 1),
                            b.match(/,([mwd0](&[ol\d]+)?|[olx]s)(&[ol\d]+)?$/) && (m = 0),
                            f && x) {
                                let m = s.optimal;
                                if ((f.match(/&\d+/) || x.match(/&\d+/)) && (m = !0),
                                m) {
                                    let m = O({
                                        from: f,
                                        to: x,
                    ...
                    (s ? v(s.selector, s.sex) : []).forEach((function(f) {
                        let d = [f]
                          , m = s.sex;
                        x.reverse && (d = i(f, m),
                        m = f.match(/([fhs1](&[ol\d]+)?|[olx]b)$/) ? 1 : 0),
                        "chain" == x.type ? d.forEach((function(f) {
                            let x = E(f, s.sex);
                            x && b.push(x)
                        }
                        )) : "pair" == x.type ? (d = i(f, s.sex),
                        d.forEach((function(s) {
                            let x = function(s, f) {
                                let x = []
                                  , d = []
                                ...
                                s = s.replace(/&\d+/, ""),
                                f = f.replace(/&\d+/, "");
                                let b = s.replace(/([ol])([bs])/, "x$2")
                                  , l = f.replace(/([ol])([bs])/, "x$2")
                                  , M = s.replace(/&[ol]/, "")
                                  , w = f.replace(/&[ol]/, "");
                                for (let o in h) {
                                    let e = o.split("#");
                                    if (e.length > 1) {
                                        let t = v(e[0])
                                          , r = v(e[1])
                                          , G = t.map((function(s) {
                                            return s.replace(/&[ol\d]+/, "").replace(/([ol])([bs])/, "x$2")
                                        }
                                        ))
                                          , c = r.map((function(s) {
                                            return s.replace(/&[ol\d]+/, "").replace(/([ol])([bs])/, "x$2")
                                        ...
                                        (t.includes(s) && r.includes(f) || t.includes(f) && r.includes(s)) && x.push(h[o][0]),
                                        (G.includes(b) && c.includes(l) || G.includes(l) && c.includes(b)) && d.push(h[o][0]),
                                        (G.includes(M) && c.includes(w) || G.includes(w) && c.includes(M)) && m.push(h[o][0])
                                    }
                                }
                                return x.length || (x = d),
                                x.length || (x = m),
    ...
    return k.data = p,
    k.dataCount = Object.keys(p).length,
    k.setMode = function(s, f) {
        a[s] = Object.assign(a[s] || {}, f)
    }
    ,
    k
}
